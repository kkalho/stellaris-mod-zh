...
> **维护轮 16（2026-09-05，本轮）**：**wave7 上公网**——#301-350 共 50 条（gameplay 薄 262→212、
> desc 薄 36→30），修复 2 处串扰（707297106 gameplay+features 错挂 872596925 迅猛龙头像；
> 1836457943 summary+features 与原文相悖——原模组是「移除银河地图仅玩 5 星团」，原文案误写成
> 「生成海量恒星」，fix_crosstalk_007.json）。C 组「Model request failed」再次复现 wave5 教训：
> **先查磁盘产物再决定重派**（本次文件已完整写入，零重派）。wave8 任务包（#351-400）已预导出。
> ⚠️ 新坑：TAT 同步脚本里的完整 SHA 必须 `git rev-parse HEAD` 取真实值，不许手拼短 SHA。
> **维护轮 17（2026-09-06，本轮）**：**wave8 上公网**——#351-400 共 50 条（gameplay 薄 212→162、
> desc 薄 30→26），修复 2 处（2436408502 gameplay+features 错挂 784249404 开局自动普查；
> 1800624677 features「中文」项原文与汉化包库均无据，移除）。**⚠️ 通道故障预案**：C 组子智能体
> 三连「Model returned no text/captcha verify failed」且零磁盘产物——与 wave5/wave7 的「报错≠没做」
...
> **维护轮 20（2026-09-06，本轮，第三档改进）**：①**同义词搜索**——term_list 驱动的查询扩展：
> 搜索词全等命中某术语的 canonical/别名时自动按整组变体做 OR 匹配（服务器 `term_variants()`
> + 快照端内嵌 65 个同义词组 `SYNON`）；实测搜「国民理念」即中「国策」类 MOD。②**内容时效提示**
> ——详情接口新增 `stale_notice`（reason=stale 原文 hash 已变 / updated 翻译确认后又有更新，
> 按日粒度比较同日不误报），前端详情页「内容时效」格 + 快照详情页同步渲染；当前全库 0 例
> （该字段是将来 MOD 更新后的守护机制）。均经本地 8099 + 公网双验证。③**EU4/VIC3 扩展与前端
> 模块化维持延后**（理由见维护轮 19/20 交接记录）。快照版已重生成（1373KB，含 wave8 修复数据
> + 同义词 + stl 字段），version.json 已随 7a2004b 推送。
> **维护轮 21（2026-09-06，本轮）**：**wave9 上公网**——#401-450 共 50 条（gameplay 薄 162→110、
> desc 薄 26→25；多降的 2 条来自 wave9 前置串扰修复的 gameplay 重写），**首次全程应用术语统一**
> （子智能体 prompt 带 canonical 清单）。**全流程首跑 run_wave.py**：export 重导（--force 让任务包
> 带上串扰修复后字段，从源头避免污染）→ 三组精做零通道故障 → merge（串扰阻断通过）→ import
> （译名报告：本批仅 30 处变体，多为复制字段历史用词）→ sync → 公网复验 → archive。
...
> **维护轮 23（2026-09-10，本轮）**：**🎉 深度精做全库收官**——wave10（#451-500）/ wave11（#501-550）
> / wave12（#551-584 收官批 34 条，两子智能体并行）三波连跑全部上公网；**gameplay<100 字与
> description<150 字双双清零（584/584 目标全部达标）**，本地+云端+公网三处一致（verify_db 健康、
> pytest 12 绿）。累计修复串扰/文案矛盾 **26 处**：wave10 三处（星战库亚特黎明克隆战争无据、光环
> 特效「提升性能」反原文、ACOT 补完张冠李戴）、wave11 两处（PD Planet View 实为可独立运行、
> Ships in Scaling 实为战斗半径适配）、wave12 两处（舰队转移实为所有权转让/租售、银河帝国税改
> 全资源征税表述）——子智能体语义级上报是精确重复检测之外的第二道网。**run_wave sync 第二处
> 硬编码修正**：expect 由「50+fix 数」改为「合并件实际条数+fix 数」（收官批 34 条被服务端 sanity
> 正确拦截后修）；detect_crosstalk 豁免名单扩展到 --files 模式；export 收官批次 C 组打印越界修复。
> 工坊快照已用收官数据重生成（**1504KB**，exported_at=2026-09-10，version.json 已推送），桌面发布
> 副本同步刷新。**Steam 发布仍卡在「创建物品 Access Denied」——登录/令牌验证均已通过，等待用户
> 确认发布账号是否拥有群星本体**（受限账号或无本体会被拒绝创建工坊物品）。
...
> **云端同步已完成（2026-09-14）**：TAT `inv-t8mb1pgamr` SUCCESS（64s）——apply 更新 2008 /
> 翻译导入 2008 / mine_compat 353 / 基线 2008/2008 / 公网 stats `total=2008, translated=2008`。
...
**2. 干到哪了？（2026-09-13/14 实测快照，本地 = 云端 = 公网一致）**

| 游戏 | MOD 数 | 已翻译 | 六字段覆盖 | 说明 |
|---|---|---|---|---|
| **群星** | **2008**（目标 2000+ ✅ **扩容收官**） | 2008/2008 | **100%** ✅ | expand wave1-20（2026-09-13）完成，创意工坊 Top 2008 全收录，A1 精做；云端已同步 |
...
| **P1** | ~~云端同步 2008 库 + 补腐化基线~~ **✅ 已完成（2026-09-14）** | TAT `inv-t8mb1pgamr` SUCCESS：更新 2008 + 翻译导入 2008 + compat 353；公网 stats total=2008/translated=2008；基线 2008/2008 |
...
4. `curl http://127.0.0.1:8080/api/stellaris/stats` 与 `curl http://150.158.24.195:8080/api/stellaris/stats` —— 本地/云端均应为 total=2008、translated=2008。本地测试服务惯例跑 8099（`python web_server_multigame.py 8099`，**用户浏览器标签可能开着它，别乱关**）
5. 读 §5.2 当前进度 —— Top 2000 扩容已收官（2008/2008，云端已同步）；当前主线 = 新入榜增补 / 深度维护 / 功能增强，流水线见 HANDOFF §8 云同步机制
