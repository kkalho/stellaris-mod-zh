...
> （584 目标清单）。**wave1-4（#1-200）已完成并上公网**：gameplay 薄 562→362、desc 薄 140→53；
> 顺带发现并修复 **4 处历史数据串扰**（相邻条目字段错挂：1616934635/2774388842/3250900527/
> 1720760712，fix_crosstalk_003/004.json，扫描方法=任务包内相邻 features 相同检测）。
> wave5 A/B 已产出存档（1a355c2，**未导入**），C 组因网络中断待补；新发现 3 处串扰待修：
> 1316044027（额外舰船组件↔飞升槽）、910355834（额外传统↔碧蓝语音）、2059474384（舰船分段↔质量中继器）。
> **续作指引**：① 补做 wave5 C 组（task_005 #235-250）② 写 fix_crosstalk_005 修 3 条 ③ 合并导入
> wave5 全部 50 条 → 推送 → r 系列同步 ④ wave6-12（#251-584）同法。⚠️ min-byte 门槛三连误拦
> （fix 文件 554B/1688B 均被 1200/2000 门槛拦）——**统一规则：门槛=实际大小的一半**。
...
| **P1** | **深度精做补洼地（进行中，维护轮 13 启动）** | 目标 584 个薄字段 MOD（gameplay<100字 / desc<150字），流水线见 §5.2；**wave1-4（#1-200）已上公网**，wave5 A/B 已产出待 C 组，剩 #251-584 ≈ 7 批 |
...
**串扰治理**（本轮发现 6 处，修 4 待修 3）：症状=某 MOD 的 gameplay/features 字段是相邻条目的内容（历史子智能体错位）。检测法：任务包内**相邻条目 features 完全相同**即嫌疑。待修：1316044027（额外舰船组件↔飞升槽）、910355834（额外传统↔碧蓝语音）、2059474384（舰船分段↔质量中继器）——照 `fix_crosstalk_004.json` 格式按各自原文重写即可。

**当前进度**：wave1-4（#1-200）✅ 上公网；wave5 A/B 已产出（git 1a355c2，未导入）、C 组待补；剩 #251-584。
