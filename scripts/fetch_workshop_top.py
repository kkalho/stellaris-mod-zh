"""抓取《群星》创意工坊订阅量排行（官方页面内嵌数据，无需 API Key）

数据源: steamcommunity.com/workshop/browse （Steam 官方页面）
页面内嵌 window.SSR.renderContext → queryData → queries[13].state.data.results
包含: 标题/作者/订阅数/收藏/星级/投票数/标签/简介/更新时间/预览图

用法: python fetch_workshop_top.py [--pages 34] [--sleep 8] [--start 1]
输出: data/workshop_top1000.json  (完整字段)
"""
...
def main():
    ...
    ap.add_argument("--pages", type=int, default=34, help="抓取页数（每页 30 条）")
    ...
    out_path = os.path.join(base, "data", "workshop_top1000.json")
